...
extern "C" {
jint Java_com_example_VoiceRecognize_VoiceRecognize_NormFeat1(JNIEnv* env,
      							      jobject thiz,
							      /*   jstring jfilename,
							      jstring jinname,
							      ...
							      jobjectArray stringArrays)
{
  ...
  const char *nativeinname = env->GetStringUTFChars(jinname,0);
...
#include "jniXcommon.hpp" 
 ...
 /*
    ...
    argv[4]=new char[strlen(nativeinname)+1];
    strcpy(argv[4], nativeinname);
 ...
 */
  try
  {
    ...
    int i;
     ...
     env->ReleaseStringUTFChars(jinname, nativeinname);
     env->ReleaseStringUTFChars(jfeaturepath, nativefeaturepath);
     if (argc > 7)
       env->ReleaseStringUTFChars(jfeaturepath, nativelabelpath);
     */

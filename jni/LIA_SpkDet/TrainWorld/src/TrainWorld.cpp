...
void featureStream(Config &config,String filename,FeatureServer *&fs,SegServer *&segServ,SegCluster *&segCluster,String labelSelectedFrames){
  ...
  try{   
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "TrainWorld", " ***excep %s **** line %d \n",e.toString().c_str(), __LINE__);
...
int trainWorld(Config& config){
  ...
  // __android_log_print(ANDROID_LOG_DEBUG, "TrainWorld", " ******* line %d \n",__LINE__);
  ...
  try{    
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "TrainWorld", "World model init from scratch");

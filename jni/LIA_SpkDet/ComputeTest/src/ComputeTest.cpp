...
//-------------------------------------------------------------------------------------------------------
WindowLLR2::WindowLLR2(Config &config){
	_set=false;
	_size=0;
	_dec=0;
	_bIdx=0;
	...
	_nClient=0;
	_llrM=NULL;
	if (config.existsParam("windowLLR2")) _set=config.getParam("windowLLR2").toBool();
	if (_set){
...
//-------------------------------------------------------------------------------------------------------
WindowLLR2::~WindowLLR2(){
	_freeMem();
}

//-------------------------------------------------------------------------------------------------------
void WindowLLR2::showConfig(){
	if (_set) cout<<"windowLLR mode size["<<_size<<"] dec["<<_dec<<"]"<<endl; 
}

//-------------------------------------------------------------------------------------------------------
unsigned long WindowLLR2::wCount(){
	return (_count);
}

//-------------------------------------------------------------------------------------------------------
void WindowLLR2::dec(unsigned long idxFrame){
...
//-------------------------------------------------------------------------------------------------------
void WindowLLR2::accLLR(unsigned long clientIdx,double llr){
	(*_llrM)((_bIdx+_count-1)%_size,clientIdx)=llr;
	(*_accLlrA)[clientIdx]+=llr;
}

//-------------------------------------------------------------------------------------------------------
double WindowLLR2::getLLR(unsigned long clientIdx){
	return (*_accLlrA)[clientIdx]/(double)_size;
}

//-------------------------------------------------------------------------------------------------------
bool WindowLLR2::isEnd(){
	return (wCount()==_size);
}
...
int ComputeTest(Config& config){
	...
	WindowLLR2 windowLLR2(config); 										// Initialize the windowLLR mode if requested
	windowLLR2.showConfig();
...
try{
	...
	while ((linep=ndx.getLine()) != NULL){                                 					// Loop on each line of the ndx input file
		...
		if (codeSelectedFrame==-1)                                                            		// The file is empty
			...
			if (windowLLR2.isSet()) windowLLR2.setNbClient(tabClientLine.nbClientLine()); // If needed reset the score/LLR window using the good number of client
			...
			while((seg=selectedSegments.getSeg())!=NULL){        				// For each of the selected segments
				...
				for (unsigned long idxFrame=0;idxFrame<seg->length();idxFrame++){               // For each frame of the segment
					...
					if (windowLLR2.isSet()) windowLLR2.dec(idxBeginFrame+idxFrame);
						for (unsigned long i=0;i<tabClientLine.nbClientLine();i++){                             // For each client, compute LLK using the winning
							...
							if (windowLLR2.isSet()) windowLLR2.accLLR(i,llkc-llkw);
						}
					if (windowLLR2.isSet() && windowLLR2.isEnd()){
						for (unsigned long i=0;i < tabClientLine.nbClientLine();i++){                  // For each client
							double LLRClient=windowLLR2.getLLR(i);
							...
							featureFileName, frameIdxToTime(windowLLR2.getIdxBegin(),frameLength),frameIdxToTime(windowLLR2.getIdxEnd(),frameLength),gender ,decision,outNist);
							if (verboseLevel>1) outputResultLine(LLRClient, tabClientLine.getClientName(i), featureFileName, frameIdxToTime(windowLLR2.getIdxBegin(),frameLength),frameIdxToTime(windowLLR2.getIdxEnd(),frameLength),gender ,decision,cout);
						}  	
					}
				}  
	...
	cout << e.toString().c_str() << endl;
		...
		__android_log_print(ANDROID_LOG_DEBUG, "CompTest", "Exception[%s]\n",
				    e.toString().c_str() );
#endif

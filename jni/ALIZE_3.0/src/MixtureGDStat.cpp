...
occ_t M::computeAndAccumulateEM(const Feature& f, double w)
{
  ...
  /***********************
#ifdef __ANDROID_API__
  __android_log_print(ANDROID_LOG_DEBUG, "MixGDStat", "-computeAndAccEM, w=%f; f=%s; sum=%f\n", w,f.toString().c_str(),sum);
#else
   printf("MixGDStat -computeAndAccEM, w=%f; f=%s; sum=%f\n", w,f.toString().c_str(),sum);
#endif
  ******************************/
...
const Mixture& M::getEM()
{
  ...
  for (c=0; c<_distribCount; c++)
  {
    ...
    if (occ > 0.0)
    {
      ...
      /**********************8
#ifdef __ANDROID_API__
      __android_log_print(ANDROID_LOG_DEBUG, "MixGDStat", "-computeall getEM, mean=%f; cov=%f; c=%lu; occ=%f; totOcc=%f \n", mean,cov,c,occ,totOcc);
#else
   printf("MixGDStat -computeall getEM, mean=%f; cov=%f; c=%lu; occ=%f; totOcc=%f \n", mean,cov,c,occ,totOcc);
#endif
      ***********************/
      d.computeAll();
    }

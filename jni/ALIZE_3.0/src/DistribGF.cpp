...
#define RAND_MAX00 2147483647.0
  for (unsigned long i=0; i< _vectSize; i++)
  {
    ...
    {
      (*_pCovMatr)(i,j) = (rand()+1.0)/(RAND_MAX00+1.0); // always > 0.0
    }
    _meanVect[i] = (double)rand()*2/RAND_MAX00 - 1.0;
#ifdef __ANDROID_API__
     __android_log_print(ANDROID_LOG_DEBUG, "MIXTURE GF", " **********meanveci  %f covmatij=%f  \n", _meanVect[i], (*_pCovMatr)(i,0));
#endif
  }
  computeAll();*/
}

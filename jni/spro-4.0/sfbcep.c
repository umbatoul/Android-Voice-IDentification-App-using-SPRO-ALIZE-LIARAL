...
/* ----- int main(int, char **) ----- */
/* ---------------------------------- */
jint Java_com_example_VoiceRecognize_VoiceRecognize_Sfbsep(JNIEnv* env,
							   jobject thiz,
							   jstring jifilename,
							   jstring jofilename)

{
  const char *nativeifilename = (*env)->GetStringUTFChars(env, jifilename,0);
  const char *nativeofilename = (*env)->GetStringUTFChars(env, jofilename,0);
  int ii;
  char *ifn, *ofn;
 /* I/O filenames                            */
  int status;                     /* error status                             */

  /*"bin/sfbcep -m -k 0.97 -p19 -n 24 -r 22 -e -D -A -F SPHERE  data/sph/$i.sph data/prm/$i.tmp.prm" */
  

  int myargc=16;
  char* myargv[16];

  myargv[0]=(char *) malloc(7);
  strcpy(myargv[0],"sfbcep");

  myargv[1]=(char *) malloc(3);
  strcpy(myargv[1],"-m");

  myargv[2]=(char *) malloc(3);
  strcpy(myargv[2],"-k");

  myargv[3]=(char *) malloc(5);
  strcpy(myargv[3],"0.97");

  myargv[4]=(char *) malloc(5);
  strcpy(myargv[4],"-p19");
 
  myargv[5]=(char *) malloc(3);
  strcpy(myargv[5],"-n");

  myargv[6]=(char *) malloc(3);
  strcpy(myargv[6],"24");

  myargv[7]=(char *) malloc(3);
  strcpy(myargv[7],"-r");

  myargv[8]=(char *) malloc(3);
  strcpy(myargv[8],"22");

  myargv[9]=(char *) malloc(3);
  strcpy(myargv[9],"-e");

  myargv[10]=(char *) malloc(3);
  strcpy(myargv[10],"-D");

  myargv[11]=(char *) malloc(3);
  strcpy(myargv[11],"-A");

  myargv[12]=(char *) malloc(3);
  strcpy(myargv[12],"-F");

  myargv[13]=(char *) malloc(5);
  strcpy(myargv[13],"WAVE");


  /*"bin/sfbcep -m -k 0.97 -p19 -n 24 -r 22 -e -D -A -F SPHERE  data/sph/$i.sph data/prm/$i.tmp.prm" */

  myargv[14]=(char *) malloc(strlen(nativeifilename)+1);
  strcpy(myargv[14], nativeifilename);

  myargv[15]=(char *) malloc(strlen(nativeofilename)+1);
  strcpy(myargv[15], nativeofilename);

  int read_args(int, char **);
  int process(const char *, const char *);

  /* ----- process command line ----- */
  if (read_args(myargc, myargv))
    return(1);
  ...
  if ((status = process(nativeifilename, nativeofilename)) != 0) {
    fft_reset(); dct_reset();
    return(status);
  }
  ...
  (*env)->ReleaseStringUTFChars(env, jifilename, nativeifilename);
  (*env)->ReleaseStringUTFChars(env, jofilename, nativeofilename);
 ...
 */
int process(const char *ifn, const char *ofn)
{
  sigstream_t *is;
  spfstream_t *os;
 ...
 *  -h, --help
 */
int read_args(int myargc, char **myargv)
{
  int c, i;
  char *p;
  ...
  while ((c = getopt_long(myargc, myargv, "F:f:x:BI:O:k:l:d:w:n:a:mi:u:b:p:r:es:ZRL:DANvVh", longopts, &i)) != EOF)

...
int myidx[]={
2, 3, 4, 5, 7, 9, 14, 17, 18, 20, 21, 23, 26, 27, 30, 31, 0, 1, 4, 
7, 10, 12, 13, 15, 16, 17, 18, 20, 25, 26, 29, 30, 0, 4, 5, 6, 7, 8,
 9, 12, 13, 14, 16, 17, 18, 20, 21, 24, 26, 27, 0, 4, 7, 10, 11, 12,
 13, 16, 17, 18, 19, 20, 22, 24, 28, 31, 0, 2, 3, 4, 7, 13, 14, 20, 
24, 25, 31, 0, 2, 6, 7, 10, 11, 12, 16, 21, 22, 23, 29, 0, 1, 3, 4, 
11, 12, 16, 18, 20, 21, 24, 29, 31, 5, 7, 8, 11, 12, 13, 14, 16, 17,
 19, 21, 22, 28, 30, 0, 3, 4, 5, 7, 8, 9, 10, 11, 13, 17, 18, 19, 22,
 29, 31, 0, 1, 2, 3, 6, 7, 8, 10, 12, 13, 23, 25, 0, 1, 3, 6, 8, 9, 11,
 13, 14, 17, 18, 19, 20, 24, 25, 27, 28, 29, 31, 0, 1, 2, 3, 4, 5, 6, 
10, 11, 13, 14, 16, 17, 18, 19, 20, 22, 24, 27, 28, 29, 30, 1, 5, 6, 7, 
9, 11, 14, 15, 16, 17, 18, 19, 22, 27, 28, 29, 0, 5, 6, 7, 11, 17, 18,
 22, 24, 25, 26, 28, 2, 3, 10, 11, 12, 14, 15, 20, 21, 24, 26, 30, 31,
 3, 4, 6, 8, 9, 10, 12, 13, 19, 20, 21, 24, 26, 30, 31, 0, 3, 5, 6, 8,
 9, 10, 11, 13, 14, 15, 16, 17, 19, 20, 21, 23, 24, 25, 26, 27, 29, 31,
 0, 1, 2, 4, 8, 10, 12, 14, 15, 18, 19, 21, 23, 26, 27, 28, 30, 0, 2, 6,
 12, 15, 16, 17, 18, 19, 20, 22, 25, 26, 28, 30, 31, 0, 3, 5, 7, 9, 11,
 12, 14, 19, 21, 22, 23, 26, 28, 0, 6, 7, 9, 12, 13, 14, 15, 17, 18, 23,
 24, 25, 26, 30, 31, 1, 2, 3, 5, 9, 13, 16, 17, 18, 21, 22, 23, 24, 25,
 26, 27, 28, 30, 1, 4, 6, 7, 9, 12, 13, 14, 16, 17, 18, 19, 25, 27, 29,
 0, 4, 5, 7, 8, 9, 10, 11, 13, 15, 17, 19, 22, 25, 26, 28, 29, 31, 1, 2,
 3, 8, 12, 14, 16, 18, 19, 21, 22, 24, 25, 28, 29, 31, 3, 5, 6, 8, 9, 11, 13,
 14, 15, 16, 17, 18, 20, 21, 22, 24, 26, 27, 28, 29, 31, 0, 1, 10, 13, 20, 23, 
27, 28, 29, 30, 31};

int myidxl=sizeof(myidx)/sizeof(int);
int bagfrcall=0;
int iiidx=0;
...
bool baggedFrame(double baggedFrameProbability, int ix){
  // return (drand48()< baggedFrameProbability);


  //hussein : added 3 lines and const array above, 
  // uncomment rand() and return (res<baggedFrameProb) lines

  if (bagfrcall == myidxl) bagfrcall=0;
  //  if (bagfrcall > myidx[iiidx]) {
  //bagfrcall=1;
  //if (myidx[iiidx]==0) 
  //  {iiidx++; return (true);}
  //return (false);
  //}
  //if (bagfrcall++ == myidx[iiidx]) {iiidx++; return(true);}
  if (ix == myidx[iiidx]) {iiidx++; return(true);}
  return(false);
  ...
  while(!end){
    ...
    if (length>0){
		for (unsigned long idx=0;idx<nbBagged;idx++) // For each component
		  if(baggedFrame(baggedProbability, idx)) {
			Seg &newSeg=segServerOutput.createSeg(beginSeg,length,idx,seg->string(),seg->sourceName());       
			baggedSeg.add(newSeg);
			if (debug || verbose) cout << "bagged - Adding in bagged["<<idx<<"] the seg ["<<seg->sourceName()<<"]"<<newSeg.begin()<<" "<<newSeg.length()<<endl;   
// #ifdef __ANDROID_API__
// 			__android_log_print(ANDROID_LOG_DEBUG, "GeneralTools", 
// 					    "bagged - Adding in bagged[%lu] the seg [%s], %lu, %lu \n", 
// 					    idx, seg->sourceName().c_str(), newSeg.begin(), newSeg.length()); 
// #endif
...
// #ifdef __ANDROID_API__
//     __android_log_print(ANDROID_LOG_DEBUG, "-1 Bagged segments", "Initial frames[%lu] Selected frames[%lu],  pselected[%f]\n", total, selected, percent);
// #endif

	cout <<"1 Bagged segments, Initial frames["<<total<<"] Selected frames["<<selected<<"] % selected["<<percent<<"]"<<endl;
  }
}
/*
...
void baggedSegments(SegCluster &selectedSegments,SegCluster &baggedFrameSegment,double baggedProbability,
  ...
  while(!end){
#ifdef __ANDROID_API__
//       __android_log_print(ANDROID_LOG_DEBUG, "BagSeg536", "bagged - current input seg [%lu, %lu]\n", beginSeg, lengthSeg);
#else
      //
    if (debug) 
      cout << "bagged, current input seg ["<<beginSeg<<","<<lengthSeg<<"]"<<endl;
#endif
    unsigned long  verifyLength=correctedLength(lengthSeg,minimumLength,maximumLength);
    ...
    unsigned long length=0;
// #ifdef __ANDROID_API__
//       __android_log_print(ANDROID_LOG_DEBUG, "BagSeg546", "bagged - current input seg %lu %lu\n\n", lengthSeg, verifyLength);
// #else
//     cout << "BagSeg546  lengthSeg="<<lengthSeg<<", verifyLength="<<verifyLength<<endl;
// #endif
    ...
    if ((length>0)){
      //&&(baggedFrame(segBaggedProbability,0))) {

      //if (debug) 
      cout << "bagged562 inside 562\n"<<endl;

      Seg &newSeg=segServerOutput.createSeg(beginSeg,length,0,seg->string(),seg->sourceName());       
      baggedFrameSegment.add(newSeg);
#ifdef __ANDROID_API__
     //  __android_log_print(ANDROID_LOG_DEBUG, "BagSeg564", "bagged - Adding the seg [%s]%lu %lu\n\n", seg->sourceName().c_str(), newSeg.begin(), newSeg.length());
#else
      if (debug) 
	cout << "bagged564 - Adding the seg ["<<seg->sourceName()<<"]"<<newSeg.begin()<<" "<<newSeg.length()<<endl;   
#endif
    }
    if (moveSeg){
      seg=selectedSegments.getSeg();
  ...
  if (verbose){
    ...
    double percent=(double)selected*100/(double) total;
// #ifdef __ANDROID_API__
//     __android_log_print(ANDROID_LOG_DEBUG, "-2 Bagged segments", "Initial frames[%lu] Selected frames[%lu],   pselected[%f]\n", total, selected, percent);
// #endif
//     cout <<"2 Bagged segments, Initial frames["<<total<<"] Selected frames["<<selected<<"] % selected["<<percent<<"]"<<endl;
  }
}
...
void baggedSegments(SegCluster &selectedSegments,SegCluster &baggedSelected,SegCluster &baggedUnselected,double baggedProbability,
  ...
  while(!end){
    ...
    if (length>0){
    	Seg &newSeg=segServerOutput.createSeg(beginSeg,length,0,seg->string(),seg->sourceName());       
    	if(baggedFrame(segBaggedProbability,0)){
       	 baggedSelected	.add(newSeg);
#ifdef __ANDROID_API__
      // __android_log_print(ANDROID_LOG_DEBUG, "BagSeg631", "baggedSelected - Adding the seg [%s]%lu %lu\n\n", seg->sourceName().c_str(), newSeg.begin(), newSeg.length());
#else
      if (debug) 
      cout << "baggedSelected 631 - Adding the seg ["<<seg->sourceName()<<"]"<<newSeg.begin()<<" "<<newSeg.length()<<endl;   
#endif
    	}
    	else{
	  ...
	  //      	  if (debug) 
#ifdef __ANDROID_API__
//       __android_log_print(ANDROID_LOG_DEBUG, "BagSeg641", "baggedUnselected - Adding the seg [%s]%lu %lu\n\n", seg->sourceName().c_str(), newSeg.begin(), newSeg.length());
#else
      if (debug) 
      cout << "baggedUnselected - Adding the seg ["<<seg->sourceName()<<"]"<<newSeg.begin()<<" "<<newSeg.length()<<endl;   
#endif
  ...
  if (verboseLevel>1){
    ...
    double percent=(double)selected*100/(double) total;
// #ifdef __ANDROID_API__
//     __android_log_print(ANDROID_LOG_DEBUG, "-3 Bagged segments", "Initial frames[%lu] Selected frames[%lu],   pselected[%f]\n", total, selected, percent);
// #endif
//     cout <<"3 Bagged segments, Initial frames["<<total<<"] Selected frames["<<selected<<"] % selected["<<percent<<"]"<<endl;
  }
}
...
void cms(String & featureFileName,FeatureServer &fs,Config &config) {
	...
	//	__android_log_print(ANDROID_LOG_DEBUG, "General", "  ^^^^^   \n");

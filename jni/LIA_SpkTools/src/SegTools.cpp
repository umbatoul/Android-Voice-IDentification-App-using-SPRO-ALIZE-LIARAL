...
void showCluster(SegCluster& cluster){
  ...
  while((seg=cluster.getSeg())!=NULL){
    ...
    //    __android_log_print(ANDROID_LOG_DEBUG, "ShowCluster", "   %s  \n", seg->sourceName().c_str());
		...
		LabelServer& labelServer,SegServer& segmentsServer)
{
  ...
  //  __android_log_print(ANDROID_LOG_DEBUG, "ADDsegment", " File %s  Name %s\n", file.c_str(), name.c_str());
...
void  loadLabelFile(SegCluster &cluster,String fileName,String path, String extension,Config &config)
{
  ...
  //  __android_log_print(ANDROID_LOG_DEBUG, "loadLabelFile", " ^^^^^file  %s path %s extension %s \n", fileName.c_str(), path.c_str(), extension.c_str());
...
labelServer,Config& config)
{
  ...
  //  __android_log_print(ANDROID_LOG_DEBUG, "loadCluster", " Load Cluster Done ^^^^^file  %s  \n", fileName.c_str());
...
void initializeClusters(const XLine& listFiles,SegServer& segmentsServer,LabelServer& labelServer,Config& config)
{
  ...
  while ((filep=listFiles.getElement()) != NULL){   // for each label file
    ...
    //    __android_log_print(ANDROID_LOG_DEBUG, "initializeCluster", " Done load cluster file reading ^^^^^file  %s  \n", file.c_str());
...
config)
{
   ...
   if (file.endsWith(".lst")){                                                                        // If the file parameter is the name of a XList file
    ...
    loadClusterFile(file,segmentsServer,labelServer,config); 
    //    __android_log_print(ANDROID_LOG_DEBUG, "initializeCluster", " 2- Done load cluster file reading ^^^^^file  %s  \n", file.c_str());
...
// __android_log_print(ANDROID_LOG_DEBUG, "initializeCluster", "3- Done load cluster file reading ^^^^^file  %s  \n", lblPath.c_str());

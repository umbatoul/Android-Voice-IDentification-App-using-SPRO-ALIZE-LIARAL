...
void PldaModel::initTrain(PldaDev dev,Config &config){
	 ...
	 try {
		...
		_tU			= Eigen::MatrixXd::Zero(_rankF+_rankG,1);
...
// Version EIGEN
	...
	_EhhSum = Eigen::MatrixXd::Zero(_rankF+_rankG,_rankF+_rankG);
	_xhSum = Eigen::MatrixXd::Zero(_vectSize,_rankF+_rankG);
	_tU = Eigen::MatrixXd::Zero(_rankF+_rankG,1);	// TO DO changer pour un vecteur
...
void PldaModel::mStep(unsigned long it, Config& config){
	...
	// Minimum Divergence
	_tU = _tU/_Dev.getSessionNumber();
	Eigen::MatrixXd c = _EhhSum/_Dev.getSessionNumber() - _tU*_tU.transpose();

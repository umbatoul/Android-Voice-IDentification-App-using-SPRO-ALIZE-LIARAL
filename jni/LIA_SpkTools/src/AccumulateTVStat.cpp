...
void TVAcc::_init(XList &ndx, Config &config){
	...
	_tC.setDimensions(_rankT,_svSize);
	_tC.setAllValues(0.0);
...
void TVAcc::computeAndAccumulateTVStatUnThreaded(Config& config){
	...
	//	__android_log_print(ANDROID_LOG_DEBUG, "AccumulateTV", "   GGGGGG \n");
...
void TVAcc::updateTestimate(){
	...
	for(unsigned long d=0;d<_n_distrib;d++){
		...
		tmpc=tmpC.getArray(); inva=invA.getArray(); cev=_tC.getArray();
	
		for(unsigned long i=0; i<_rankT;i++){
			for(unsigned long j=0; j<_vectSize;j++){
				for(unsigned long k=0; k<_rankT;k++){
					tmpC(i,d*_vectSize+j) += invA(i,k) *  _tC(k,d*_vectSize+j);
				}
			}
		}
	}
...
void TVAcc::estimateAandCUnthreaded(Config& config){
	...
	y=_W.getArray(); v=_T.getArray(); f_x=_statF.getArray(); aux=AUX.getArray(); invVar=_ubm_invvar.getArray(); c=_tC.getArray();n=_statN.getArray(); R=_R.getArray(); r=_r.getArray(); meanY=_meanW.getArray();
